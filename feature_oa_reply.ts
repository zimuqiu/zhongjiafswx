...
1.  **处理 \`spec_features\`**:
    -   **对于 \`spec_features\` 列表中的每一个对象**，按顺序执行：
        a. **关键指令 - 引用与事实核查 (必须严格执行)**:
            i.   **原文定位**: 首先在 \`Application File\` 中找到支持该特征的**原句**。
            ii.  **段落号获取 (强制执行)**: 
                 - 任何引用的内容，**必须**找到其对应的段落编号（格式如 [xxxx]）。
                 - 请在原文中向该句之前（上方）回溯，直到找到最近的一个段落号。
                 - 仅在全文确实无任何段落编号的情况下，才允许使用章节名称。
            iii. **最终校验**: 在生成文本前，再次确认：内容 "\${full_sentence}" 是否真的位于你引用的位置？如果对应不上，请修正位置描述。
            iv.  记录其所在的**完整句子 (\`full_sentence\`)** 和 **位置描述 (\`location_desc\`)**。
        b. **生成文本 (必须生成两条)**:
            i.   **修改动作**: \`\${counter}. 在上述新的权利要求1中补入“\${feature_object.feature}”。\`
            ii.  **修改依据**: \`修改依据：根据\${location_desc}记载的“\${full_sentence}”，可以直接地、毫无疑义地确定上述补入内容。\`
...
2.  **最高优先级 - 引用准确性与防幻觉机制**：
    - 当引用对比文件或本申请说明书时，**必须**先摘录原文。
    - **段落号检查 (强制执行)**: 任何引用的内容，**必须**找到其对应的段落编号（格式如 [xxxx]）。请在原文中向该句之前（上方）回溯，直到找到最近的一个段落号。
    - **无号处理**: 只有在全文确实无任何段落编号的情况下，才允许使用“说明书具体实施方式部分”或“说明书第X页”来指代。
    - **引用格式**: “在对比文件X中，根据其[准确位置]的记载‘[引用原文]’可知……”。
    - **严禁**引用“权利要求书”作为现有技术的证据来源。
