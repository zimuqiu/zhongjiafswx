...
const renderHistoryList = () => {
    ...
    const searchBar = `
        <div class="mb-6 relative">
            ...
            <input type="text" id="substantive-history-search" placeholder="搜索历史记录 (文件名、日期)..." class="w-full pl-10 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all">
        </div>
    `;
    ...
    return `
        <div class="w-full max-w-5xl mx-auto">
            ...
            ${searchBar}
            <div class="space-y-4" id="substantive-history-list">
                ${history.map(entry => `
                ...
                `).join('')}
                <div id="substantive-history-no-results" class="hidden text-center text-gray-500 dark:text-gray-400 py-4">无搜索结果</div>
            </div>
        </div>
    `;
};
...
const attachEventListeners = () => {
    ...
    pageElement.addEventListener('input', (e) => {
        ...
        if (target.id === 'substantive-history-search') {
            ...
            const noResults = pageElement.querySelector('#substantive-history-no-results');
